define(["exports", "react", "react-dom", "@beyond-js/dashboard-lib/models/js", "@beyond-js/dashboard/unnamed/components/core/code", "@beyond-js/dashboard/unnamed/components/binder/code", "@beyond-js/dashboard/unnamed/workspace/context/code"], function (_exports, React, ReactDOM, _js, _code, _code2, _code3) {
  ...
  _exports.useDsNotificationContext = _exports.OLDDSNotifications = void 0;
  const {
    beyond
  } = globalThis;
  ...
  function NotificationItem({
    ...
    const openSource = () => {
      ...
      if (!item.bundle.processors.has(processorName)) {
        console.log("No existe el bundle para abrir el archivo");
        return;
      }
  ...
  *****************/
  ...
  const OLDDSNotifications = new class extends _js.ReactiveModel {
    get total() {
    ...
    _update(notifications, specs) {
      ...
      console.warn('recent before', recent, ids); //Removemos los recientes que no vengan en las notificaciones nuevas
      ...
      return; //Actualizamos los items de notificaciones que esten en recientes

      const keys = [...recent.keys()];
      const items = notifications.filter(item => keys.includes(item.id));
      console.log('items', items, specs); // if (!items.length) {
    ...
    register(notifications, specs) {
      ...
      const id = specs.module?.id ?? specs.applicationId; //No hay notificaciones, limpiamos los mapas
  ...
  ************/
  ...
  _exports.OLDDSNotifications = OLDDSNotifications;
  ...
  *************/
  ...
  const DSNotifications = new class extends _js.ReactiveModel {
    ...
    _register(notifications, specs) {
      // console.log('_register', notifications.length, notifications, specs)
    ...
    _update(notifications, specs) {
      // console.log('_update', notifications.length, notifications, specs)
    ...
    setNotifications() {
      ...
      this.recent.forEach((recent, key) => {
        ...
        if (!iterator) {
          // console.warn('El SET esta vacio')
          return;
        }
        ...
        if (this.notifications.has(moduleId)) {
          ...
          }; // console.log('3this.notifications.set',{...get.getters(), ...params})
        ...
        if (recent.size === 1) {
          // console.log('2this.notifications.set', item)
        ...
        }; // console.log('1this.notifications.set',  {...item.getters(), ...params})
    ...
    register(notifications, specs) {
      ...
      if (!notifications || notifications instanceof Array && !notifications.length) {
        if (!this.recent.has(specs.id)) return; // console.warn('recent delete', this.recent, specs.id)
      ...
      if (notifications instanceof Array) {
        // console.warn('Notificaciones Generales', this.recent, specs.id)
      ...
      const {
        ...
        overwrites,
        dependencies
      } = notifications; // console.warn('Notificaciones Diagnostics', this.recent, specs.id)
