...
var __toCommonJS = mod => __copyProps(__defProp({}, "__esModule", {
  value: true
}), mod); // .beyond/uimport/temp/vue/3.2.31.js
...
function findInsertionIndex(id) {
  ...
  while (start < end) {
    ...
    const middleJobId = getId(queue2[middle]);
...
function queueJob(job) {
  if ((!queue2.length || !queue2.includes(job, isFlushing && job.allowRecurse ? flushIndex + 1 : flushIndex)) && job !== currentPreFlushParentJob) {
    if (job.id == null) {
      queue2.push(job);
    } else {
      queue2.splice(findInsertionIndex(job.id), 0, job);
    }
...
function flushJobs(seen) {
  ...
  queue2.sort((a, b) => getId(a) - getId(b));
  const check = true ? job => checkRecursiveUpdates(seen, job) : NOOP;

  try {
    for (flushIndex = 0; flushIndex < queue2.length; flushIndex++) {
      const job = queue2[flushIndex];
    ...
    if (queue2.length || pendingPreFlushCbs.length || pendingPostFlushCbs.length) {
      flushJobs(seen);
    }
  }
}
...
function watchPostEffect(effect2, options) {
  return doWatch(effect2, null, true ? Object.assign(options || {}, {
    flush: "post"
  }) : {
    flush: "post"
  });
  ...
  const formatter = {
    header(obj) {
      ...
      if (obj.__isVue) {
        ...
        return ["div", {}, ["span", vueStyle, isShallow2(obj) ? "ShallowReactive" : "Reactive"], "<", formatValue(obj), `>${isReadonly(obj) ? ` (readonly)` : ``}`];
...
var compatUtils = null; // ../../../node_modules/@vue/runtime-dom/dist/runtime-dom.esm-bundler.js
