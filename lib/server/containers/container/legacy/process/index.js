...
module.exports = class {
    ...
    constructor(specs) {
        this.#specs = specs;
        const {id, dashboard} = specs;
        if (!id || !specs.package) throw new Error('Invalid specification');
        ...
        this.#name = `backend@${id}-${dashboard ? 'dashboard' : 'main'}`;
    }

    #change = () => ipc.events.emit('data-notification', {
        type: 'record/field/update',
        table: 'bees',
        field: 'status',
        value: this.#status,
        id: `${this.#specs.is}/${this.#specs.id}`,
        time: Date.now()
    });
    ...
    #initialise = () => {
        ...
        const options = {cwd: this.#specs.path, execArgv: ['--enable-source-maps']};
        ...
        this.#process.on('exit', code => {
            const {package: pkg, dashboard, is} = this.#specs;
            dashboard && console.log(`Process of ${is} "${pkg}" exited event with code: ${code}`.red);
