...
export class Routing extends Events {
    ...
    readonly #history: BeyondHistory;
    get history() {
        return this.#history;
    }
    ...
    constructor() {
        super();
        let configured = (<any>globalThis).__beyond_config.application.routing?.mode;
        ...
        !serverside && ![0, 1].includes(routingMode) &&
        (routingMode = location.protocol === 'file:' ? RoutingMode.Hash : RoutingMode.Pathname);

        this.#mode = routingMode;

        this.#history = !serverside && new BeyondHistory(this, RoutingMode);
