module.exports = class extends global.ProcessorSinglyCode {
    ...
    _buildSource(compiled) {
        let {code, map, template, templateMap, url, hash} = compiled;

        const ims = new Map();

        // Set template IM
        // Transform template script to CJS
        const cjsTemplate = require('./cjs')(template, templateMap);
        ...
        const scriptCjs = require('./cjs')(code, map);
        ...
        const facadeIM = {id, url, hash, code: facadeCode};
        ims.set(id, facadeIM);
