module.exports = class extends global.ProcessorAnalyzer {
    ...
    // The analyzer must expose a Source class
    #AnalyzedSource = require('./source');
    get AnalyzedSource() {
        return this.#AnalyzedSource;
    }
    ...
    _analyze(source) {
        ...
        const bridge = bundle.name === 'bridge' ? require('./bridge')(source) : void 0;
        const is = bundle.name === 'bridge' && platform !== 'backend' ? 'bridge' : 'file';
        ...
        const _interface = require('./interface')(source.relative.file, content);
        return new this.#AnalyzedSource(this.processor, 'source', source, _interface, bridge?.info, content);
    }
