module.exports = class extends global.ProcessorAnalyzerDependencies {
    ...
    _update() {
        ...
        // Processor 'ts' requires '@beyond-js/kernel/bundle/ts' as a dependency, except itself
        (() => {
            const core = '@beyond-js/kernel/bundle/ts';

            const {bundle} = this.processor.specs;
            if ([core].includes(bundle.resource) || updated.has(core)) return;
