module.exports = class extends global.Dependencies {
    ...
    constructor(packager) {
        const {hash} = packager.analyzer;
        super(packager, hash, require('./dependency'));
    }
    ...
    _update() {
        ...
        analyzer.files.forEach(source => source.dependencies.forEach((info, resource) =>
            setDependency(resource, info.is, source)));

        analyzer.extensions.forEach(source => source.dependencies.forEach((info, resource) =>
            setDependency(resource, info.is, source)));
        ...
        const {bundle} = this.container.specs;
        ![core, hydrator].includes(bundle.resource) && setDependency(core, 'import');
