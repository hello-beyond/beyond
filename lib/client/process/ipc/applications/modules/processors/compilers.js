module.exports = class {
    ...
    async get(ids) {
        ...
        for (const processor of collection.values()) {
            ...
            const {general, files, dependencies, overwrites} = processor.compiler.diagnostics.toJSON();
            const diagnostics = {
                general: processor.formatErrors(general, 'diagnostics-general'),
                files: processor.formatErrors(files, 'diagnostics'),
                overwrites: processor.formatErrors(overwrites, 'diagnostics'),
                dependencies: processor.formatErrors(dependencies, 'diagnostics')
            };

            console.log('compiler', processor.id, processor.compiler.dp, diagnostics)
