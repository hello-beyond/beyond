...
class Processor extends (require('./base')) {
    ...
    get compiler() {
        return this.#processor.compiler;
    }
    ...
    async _initialise() {
        ...
        if (!global.processors.has(name))
            return this._done(`Processor "${name}" is not registered`);
        ...
        const distribution = {dashboard: true, key: 'dashboard', platform: 'web'};
