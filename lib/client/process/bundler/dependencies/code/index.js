...
module.exports = class extends DynamicProcessor {
    ...
    // The @beyond-js/kernel/bundle (beyond bundle manager)
    #bbm;
    get bbm() {
        return this.#bbm;
    }
    ...
    #process() {
        ...
        dependencies.forEach((dependency, resource) => {
            if (!dependency) throw new Error(`Dependency "${resource}" is not defined`);
            ...
            if (this.#container.is === 'transversalPackager' && kind === 'transversal') {
                ...
                if (this.#container.transversal.name === bundle.name) return;
            }

            const name = `dependency_${count}`;

            // Register the name of the dependency of beyond bundle manager
            resource === '@beyond-js/kernel/bundle' && (this.#bbm = name);
