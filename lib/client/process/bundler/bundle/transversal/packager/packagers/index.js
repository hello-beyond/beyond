...
module.exports = class extends DynamicProcessor {
    ...
    #process(collection, updated, errors) {
        ...
        collection.forEach(container => {
            ...
            if (bundle.container.is === 'module' && !bundle.container.platforms.has(platform)) return;

            // If the container is a library, the start bundles can only be included in web and ssr platforms
            if (bundle.container.is === 'library' &&
                bundle.name === 'start' && !['web', 'android', 'ios', 'ssr'].includes(platform)) return;
