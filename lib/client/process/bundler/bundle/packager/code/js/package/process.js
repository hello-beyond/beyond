 ...
 * @return {object}
 */
module.exports = function (jscode, hmr, transversal, mode, specs) {
    'use strict';
    ...
    const kernel = bundle.resource === '@beyond-js/kernel/bundle/ts';
    const core = bundle.resource === '@beyond-js/kernel/core/ts';
    const widgets = [
        '@beyond-js/widgets/render/ts',
        '@beyond-js/widgets/layout/ts',
        '@beyond-js/widgets/routing/ts',
        '@beyond-js/widgets/application/ts'].includes(bundle.resource);
    ...
    if (!kernel) {
        if (!transversal || hmr) {
            ...
            if (!hmr) {
                // Create the bundle
                // var __Bundle because the ims of the legacy projects are not scoped
                const vars = 'Bundle: __Bundle' + (externals ? ', externals' : '');
                sourcemap.concat(`const {${vars}} = require('@beyond-js/kernel/bundle/ts');`);
                sourcemap.concat(`const __pkg = new __Bundle("${bundle.resource}")${pkgProperty};`);
            }
            else {
                // Get the bundle instance
                const vars = 'instances' + (externals ? ', externals' : '');
                sourcemap.concat(`const {${vars}} = require('@beyond-js/kernel/bundle/ts');`);
                sourcemap.concat('const __pkg = ' +
                    `require('@beyond-js/kernel/bundle/ts').instances.get("${bundle.resource}")${pkgProperty}`);
            }
