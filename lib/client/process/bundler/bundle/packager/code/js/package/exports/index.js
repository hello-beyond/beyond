 ...
 * @param kernel {boolean} Is it the @beyond-js/kernel/core or @beyond-js/ss-hydrator/main module?
...
module.exports = function (packager, hmr, kernel, sourcemap, transversal) {
        ...
        ims?.forEach(im => {
            ...
            exports?.forEach(({name, from}) => {
                ...
                if (!hmr && !transversal) {
                    ...
                    declaration.add(n);
                    exp.module += kernel ? `${tab}${n} = ${require};\n` :
    ...
    if (!hmr && !transversal) {
        ...
        const params = kernel ? 'require' : '{require, prop, value}';
