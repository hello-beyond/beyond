...
module.exports = class extends DynamicProcessor {
    ...
    // The hashes includes the extensions root hashes required
    #hashes = {};
    get hashes() {
        return this.#hashes;
    }
    ...
    get updated() {
        const hashes = this.children.get('processor.hashes').child;
        return this.#hashes.inputs === hashes.inputs;
    }
    ...
    _prepared(require) {
        ...
        // Change events can come from more than one child (eg processor.hashes, analyzer, etc)
        // and due to the asynchronous nature, one or more of them may not be up to date. 
        // Therefore, it is necessary to verify that the children required for processing are up to date.
        const {analyzer} = this;
        if (analyzer && analyzer.hashes.sources !== hashes.sources) return false;
        ...
        const {extended} = this;
        if (extended && require(extended)) {
            let prepared = true;
            extended.hashes.forEach(hash => prepared = prepared && hash === hashes.sources);
