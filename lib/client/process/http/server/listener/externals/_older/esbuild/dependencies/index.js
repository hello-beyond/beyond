module.exports = function (bundle, inputs, application) {
    ...
    const errors = [];
    dependencies.forEach((data, dependency) => {
        const reference = data.root ? data.root : data;
        // code = 2 means that the package exists, but its subpath was not found
        // This is the case of "object_assign" or "lodash"
        reference.errors?.code === 1 && errors.push(`Dependency "${dependency}" cannot be solved`);
    });
