 ...
 *      . code = 1, the package was not found
 *      . code = 2, the package exists, but its subpath was not found
 *
...
module.exports = function (input, application) {
    ...
    if (resolved.errors || resolved.resource || !resolved.json) {
        return {errors: [`Package "${pkg}" not found`], code: 1};
    }
    ...
    if (!json.exports) {
        if (!json.module && !json.main) {
            return {errors: [`Resource "${resource}" does not specify the module entry in the package.json`], code: 2};
        }
    ...
    if (!resources.has(resource)) {
        return {errors: [`Resource "${resource}" not found`], code: 2};
    }
