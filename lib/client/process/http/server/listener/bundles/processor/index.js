module.exports = async function (application, distribution, bundle, resource) {
    ...
    if (resource.diagnostics) {
        return hmr.valid ?
            new global.Resource({content: 'Processor is not reporting any errors or warnings', extname: '.html'}) :
            new global.Resource({'500': await require('./diagnostics')(processor, hmr), extname: '.html'});
    }
    else {
        return hmr.valid ?
            new global.Resource({
                content: hmr.code ? require('../sourcemap')(hmr.code, hmr.map) : '',
                extname: '.js'
            }) :
