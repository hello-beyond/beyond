...
module.exports = async function (response, resource, application) {
    try {
        ...
        form.parse(resource.request, async (err, fields, files) => {
            ...
            let path = await require('./path')(fields.id, fields.type, application);
            if (!path) {
            ...
            if (promises.length === files.images.length) {
                ...
                const images = files.images.map(image => {
                    ...
                    return {
                        ...
                        path: image.path, pathname: `${ROUTE}/${name}`
                    }
                });
