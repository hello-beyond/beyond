...
module.exports = (application, distribution) => async function (request, response) {
    ...
    try {
        ...
        if (url.pathname === '/uploader') {
            await require('./uploader')(response, {request: request}, application);
            return;
        }

        let resource;

        resource = await require('./static')(application, url);
        ...
        if (url.pathname.startsWith('/packages/')) {
            respond(new global.Resource({'404': 'Package not found'}));
            return;
        }
