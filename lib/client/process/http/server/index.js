...
module.exports = class extends DynamicProcessor {
    ...
    async _process(request) {
        ...
        if (!this.#specs.port && !distribution.port) {
        ...
        if (this.#server && distribution.port && distribution.port === this.#port) return;
        ...
        if (!port && distribution.port) {
            port = distribution.port;
            const available = await ipc.exec('main', 'ports.check', distribution.port);
            if (this._request !== request) return;
            if (!available) return done('closed', [`Port "${distribution.port}" is unavailable`]);
        }
        else if (!port) {
