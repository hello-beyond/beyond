...
module.exports = class extends DynamicProcessor {
    ...
    async process(key, distribution) {
        ...
        const {properties, uri, language, attrs} = this.get(key);
        ...
        const url = (() => {
            ...
            if (attrs?.size) {
                const names = [...attrs.keys()].join(',');
                attrs.size && (qs += `&attrs=${names}`);
                attrs.forEach((value, param) => qs += `&attrs.${param}=${value}`);
            }
            ...
            const process = language => {
                if (properties.is === 'page') {
                    ...
                    uris.forEach(uri => {
                        ...
                        const props = {resource: bundle.resource, properties, uri};
                    ...
                    const resource = `${widget}.${require('./checksum')(key)}.js`;
                    const props = {resource: bundle.resource, properties};
                    language && (props.language = language.slice(0, 2));
                    ...
                    items.forEach((item, index) => {
                        ...
                        const resource = `${widget}.${require('./checksum')(key)}.js`;
                        const props = {resource: bundle.resource, properties, attributes: [...iattrs]};
                        language && (props.language = language.slice(0, 2));
