...
module.exports = async function (bundle, path) {
    const processDeclaration = async (declaration, tsConfig) => {
        ...
        const {name, container} = packager.bundle;
        if (!valid || errors.length) return;

        try {
            const dest = p.join(path, container.pathname, name, `${name}.d.ts`);
            await fs.save(dest, code, 'utf8');
            const destTs = p.join(path, container.pathname, name, `tsconfig.json`);
