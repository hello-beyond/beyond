...
module.exports = async function (extname, specs) {
    ...
    const exported = p.join(modulePathname, bundle.rname + (language ? `.${language}` : ''));
    const entry = exported.replace(/\\/g, '/');

    const bName = bundle.rname + (language ? `.${language}` : '');
    let items = {bundleRname: bName, extensions: []};
    ...
    exports.set(bundle.resource, distribution.npm ? items : entry);
    const target = distribution.npm ?
                   p.join(path, module.pathname, `${exported}${extname}`) :
                   p.join(path, `${exported}${extname}`);
