...
module.exports = class {
    ...
    async process() {
        ...
        const projectStatic = await (require('./statics'))(builder, distribution, paths.www);

        // Copy externals resources (node dependencies resources)
        await (require('./externals'))(builder, distribution, paths.www);
        ...
        const modulesStatic = await modules.process(builder, distribution, paths.www, uglifier);

        // Save the package.json
        if (distribution.npm || !['web', 'android', 'ios'].includes(distribution.platform)) {
            ...
            modules.exported.forEach((bundle, exp) => {
                exported[`./${exp}`] = !distribution.npm ? `./${exp}/${bundle}.js` : {
                    amd: `./${exp}/${bundle}.amd.js`,
                    import: `./${exp}/${bundle}.mjs`, module: `./${exp}/${bundle}.mjs`,
                    types: `./${exp}/${bundle}.d.ts`, require: `./${exp}/${bundle}.cjs.js`
                };
            });
            ...
            projectStatic.forEach(i => items[cleanEntry(i)] = i);
            modulesStatic.forEach(i => items[cleanEntry(i)] = i);

            const {application: {name, scope, description, version, author, license}} = builder;
            const pJson = {
                name: `@${scope}/${name}`, scope: scope, author: author,
