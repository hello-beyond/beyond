...
module.exports = async function (resource, distribution) {
    ...
    if (resource.package !== application.package && !resource.name) {
        if (!libraries.has(resource.id)) {
            const error = `Library "${resource.package}" does not exist`;
            return {error};
        }
        const library = libraries.get(resource.package);

        bundle = library.bundles.has(resource.bundle) ? library.bundles.get(resource.bundle) : void 0;
        if (!bundle) {
            const error = `Library "${resource.package}" does not implement the bundle "${resource.bundle}"`;
            return {error};
        }
    ...
    if (!(application.modules.resources.has(resourceKey))) {
        const error = `Module "${resource.id}" for platform "${platform}" not found`;
        return {error};
    }
    const module = application.modules.resources.get(resourceKey);
    await module.ready;
