const {delimiter, join, relative, extname} = require("path");
...
module.exports = class File {
    ...
    setBasename(name) {
        ...
        const split = relativeData.split(delimiter);
        const file = split.splice(-1);
        this.#relative = {
            file: file.join(),
            dirname: split.length > 0 ? split.join(delimiter) : ''
        };
    ...
    async read() {
        ...
        const element = await global.utils.fs.stat(this.file);
        if (!element.isFile()) return;
        let content = readFileSync(this.file, {encoding: this.#encoding, flag: 'r'});
